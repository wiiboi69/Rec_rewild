...
{
    class AuthServer2021_new
    {
        public static AuthServer2021_new AuthServer;
        public AuthServer2021_new()
        {
            _listener = new HttpListener();
            _listener.Prefixes.Add("http://localhost:" + Program.version + "4/");
        ...
        {
            AuthServer.RegisterRoutes(true);
        }
        ...
        private void RegisterRoutes(bool reload = false)
        {
            if (reload)
            {
                Console.WriteLine("AuthServer2021 [DEBUG]: Reregistering all of the Route");
        ...
        {
            _listener.Start();
            Console.WriteLine("AuthServer2021: Listening...");
            while (true)
            {
        ...
        private void ProcessRequest(HttpListenerContext context)
        {
            ...
            if (string.IsNullOrEmpty(contentType))
                contentType = "none";
            Console.WriteLine($"AuthServer2021: Start of request.");

            Console.WriteLine($"AuthServer2021: Auth Requested: {path}");
            Console.WriteLine($"AuthServer2021: Content-Type: {contentType}");
            Console.WriteLine($"AuthServer2021: Request-Method-Type: {context.Request.HttpMethod}");
            ...
            foreach (var (regex, method, parameters) in _routeHandlers)
            {
                ...
                if (match.Success)
                {
                    ...
                    if (context.Request.ContentType == "application/json" || context.Request.ContentType == "application/x-www-form-urlencoded")
                    {
                        ...
                        Console.WriteLine($"AuthServer2021: Auth Data: {body}");
                    }
        ...
        private static void WriteResponse(HttpListenerResponse response, object response_data)
        {
            ...
            if (response_data is string responseString)
            {
                ...
                {
                    Console.WriteLine($"AuthServer2021: Auth json Response: " + responseString);
                }
                else
                {
                    Console.WriteLine($"AuthServer2021: Auth json Response Length: " + responseString.Length);
                }
            ...
            {
                Console.WriteLine($"AuthServer2021: Auth byte[] Response Length: " + responseBytes.Length);
            ...
            Console.WriteLine($"AuthServer2021: End of request.\n");
        ...
        [rewild_route_system.Route("/cachedlogin/forplatformid/{platform}/{steamid}")]
        public static string CachedLogins(string platform, string steamid)
        {
            Console.WriteLine($"game requesting cached logins for platform " + platform + " and steam id " + steamid);
            return AccountAuth.CachedLogins();
        }

        [rewild_route_system.Route("/eac/challenge")]
        public static string EACChallenge()
        {
            Console.WriteLine($"game requesting eac challenge");
            Guid randomGuid = Guid.NewGuid();
            return "\"" + randomGuid.ToString("D") + "\"";
        }

        [rewild_route_system.Route("/connect/token")]
